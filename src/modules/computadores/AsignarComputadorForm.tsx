...
import { obtenerComputadoresDisponibles, asignarComputador, crearComputador, actualizarComputador, eliminarComputador } from "../../services/computadorApi";
import { obtenerUsuarios } from "../../services/usuarioApi";
import { Usuario } from '../../types/Usuario';
import { Computador } from '../../types/Computador';
import { ConfirmModal } from "../../components/ConfirmModal";

export default function AsignarComputadorForm({ onAsignado }: { onAsignado?: () => void }) {
  ...
  const [computadorEditarId, setComputadorEditarId] = useState<number | null>(null);
  ...
  const handleAgregarCompuatador = async (e: React.FormEvent) => {
    ...
    if (nuevoComputador.trim() && nuevoNumeroSerie.trim()) {
      try {
        await crearComputador({ nombre: nuevoComputador.trim(), numeroSerie: nuevoNumeroSerie.trim() });
                      ...
                      <div className="mb-2">
                        <select
                          ...
                          onChange={e => {
                            ...
                            const comp: Computador | undefined = computadores.find((c: any) => c.id === id);
                            if (comp) {
...
>
  Eliminar computador
      ...
      <div>
        ...
        <select className="form-select" value={computadorId} onChange={(e) => setComputadorId(Number(e.target.value))} required>
          <option value="">Seleccionar equipo</option>
          {computadores.map((c: any) => (
            <option key={c.id} value={c.id}>{c.nombre} - ({c.numeroSerie})</option>
          ))}
        </select>
      </div>

      <button className="btn btn-primary mt-2" type="submit">Asignar</button>
